...
> For any UI/frontend work (components, styling, theming, icons, notifications, settings UI), also follow the
> [toxen-design-patterns](.claude/skills/toxen-design-patterns/SKILL.md) skill — it defines the design system and consistency rules.

## Change Notes (required)

**Always document major, user-relevant changes in [changenotes.md](changenotes.md)** as part of the same change — not as a follow-up.

This file is shown to end users in the "What's New" modal and the Changes panel, fetched live from `Toxen.changeLogsUrl`. Write for users, not developers.

**Where to add:** under the top `<!-- UPCOMING: x.y.z -->` section. Only sections marked `<!-- VERSION: x.y.z -->` are shown by the What's New modal, so the upcoming block stays hidden until it's flipped to `VERSION:` on release. Newest version stays at the top.

**Format** — a heading per category (`## New Features`, `## Changes`, `## Bug Fixes`, `## Boring stuff` for internal/dev notes), then one bullet per change:
...
- **Low Performance Mode** - Disables certain visual effects and fully disables the visualizer.
...
- One line per change: `- **Short Feature Name** - plain-English description.` Bold name is optional for small tweaks.
- Keep it short and friendly. Describe what the user gets, not how it was implemented. No file names, class names, or PR/issue numbers.
- Add a sub-bullet only when the user needs to know *where* to find it (`Settings > Tab > Option`) or when a feature has a couple of distinct parts.
- Skip refactors, dependency bumps, and internal cleanups — or put them under `## Boring stuff` if worth a mention.
- Don't bump the version or add a date; leave `TBD` until release.
