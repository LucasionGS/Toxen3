...
export default class Song implements ISong {
  ...
  public rainfallEffect: boolean | null; // null = use global setting
  public rainfallFrequency: number | null; // null = use global setting
  public rainfallSpeed: number | null; // null = use global setting
  public rainfallImage: string | null; // filename relative to the song directory, null = use global setting
  public rainfallImageScale: number | null; // null = use global setting
  public rainfallColor: string | null; // null = use global setting

  // Background dim setting
  public backgroundDim: number | null; // null = use global setting, 0-100 range

  // Autogenerated theme setting
  public autogeneratedTheme: boolean | null; // null = use global setting, true = enable, false = disable
  ...
  /**
   * Return the full path/URL of the per-song custom rainfall image, or null if none is set.
   * The stored value (`rainfallImage`) is a filename relative to the song directory.
   */
  public rainfallImageFile() {
    if (!this.rainfallImage) return null;
    if (Settings.isRemote()) return `${this.dirname()}/${this.rainfallImage}`;
    else if (toxenapi.isDesktop()) return toxenapi.path.resolve(this.dirname(), this.rainfallImage);
    return null;
  }
  ...
  public getEffectiveSettings(playlistId?: string): ISong {
    ...
    if (playlistId && this.playlistSettings?.[playlistId]) {
      ...
      // A playlist entry left on "<Default>" is stored as null. That means "inherit",
      // so it must fall through to the song's own value instead of replacing it.
      for (const key in playlistOverrides) {
        const value = (playlistOverrides as any)[key];
        if (value !== null && value !== undefined) (effective as any)[key] = value;
      }
      return effective;
    }
  ...
  await Toxen.background.setBackground(bg ? (bg + "?h=" + this.hash) : null);
    ...
    // Apply autogenerated theme if enabled - do this after storyboard is set up
    // Use setTimeout to ensure storyboard has fully updated
    setTimeout(() => {
...
export interface ISong {
  ...
  // Rainfall effect settings
  rainfallEffect: boolean | null; // null = use global setting
  rainfallFrequency: number | null; // null = use global setting
  rainfallSpeed: number | null; // null = use global setting
  rainfallImage: string | null; // filename relative to the song directory, null = use global setting
  rainfallImageScale: number | null; // null = use global setting
  rainfallColor: string | null; // null = use global setting

  // Background dim setting
  backgroundDim: number | null; // null = use global setting, 0-100 range

  // Autogenerated theme setting
  autogeneratedTheme: boolean | null; // null = use global setting, true = enable, false = disable
